...
# Back up optimized services and drivers
$windir = $([Environment]::GetFolderPath('Windows'))
function Backup-SystemServices {
$filePath = "$([Environment]::GetFolderPath('Windows'))\SystemRuntime\Other\OptimizedServices.reg"
...
& "$windir\SystemRuntime\Scripts\ConfigureSettingsPages.cmd" /hide account
...
function Disable-CoreIsolation {
    & ".\SystemRuntime\Scripts\ScriptWrappers\ConfigVBS.ps1" -DisableAllVBS
}
...
function Set-FileAssociations {
    ...
    Start-Process -FilePath ".\ConfigureFileAssociations.cmd" -NoNewWindow -Wait
    
    # Set default browser
    if ($Browser -in @("Brave", "LibreWolf", "Firefox", "Google Chrome")) {
        Start-Process -FilePath ".\ConfigureFileAssociations.cmd" -ArgumentList "`"$Browser`"" -NoNewWindow -Wait
    }
}

# Disable Mitigations
function Disable-Mitigations {
    Start-Process -FilePath "SystemConfig\7. Security\Mitigations\Disable All Mitigations.cmd" -ArgumentList "/silent" -NoNewWindow -Wait
}
...
function Set-PowerSettings {
    ...
    if ($DisablePowerSaving) {
        Start-Process -FilePath "SystemConfig\3. General Configuration\Power-saving\Disable Power-saving.cmd" -ArgumentList "/silent" -NoNewWindow -Wait
    }

    if ($DisableHibernation) {
        Start-Process -FilePath "SystemConfig\3. General Configuration\Hibernation\Disable Hibernation (default).cmd" -ArgumentList "/silent" -NoNewWindow -Wait
    }
