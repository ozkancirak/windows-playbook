...
for %%a in (
	...
	call "%windir%\SystemRuntime\Scripts\setSvc.cmd" %%~a 4
)

:: Seems to not exist sometimes
call "%windir%\SystemRuntime\Scripts\setSvc.cmd" BthPan 4 > nul 2>&1

:: Disable Bluetooth devices
call "%windir%\SystemRuntime\Scripts\ToggleDevices.cmd" -Silent '*Bluetooth*'

:: Disable in Send To context menu
call "%windir%\SystemConfig\4. Interface Tweaks\Context Menus\Send To\Debloat Send To Context Menu.cmd" -Disable @('Bluetooth')
